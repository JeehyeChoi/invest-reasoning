...
async function main() {
  ...
  try {
    const { rows } = await client.query(`
      ...
      LEFT JOIN ticker_company_classifications c ON c.ticker = p.ticker
      LEFT JOIN ticker_market_snapshots m ON m.ticker = p.ticker
    `);
    ...
    const result = Array.from(map.values()).sort((a, b) => {
      ...
      return a.key.localeCompare(b.key);
		...
		const outputDir = path.resolve("data", "bootstrap", "classification-tags");
		await fs.mkdir(outputDir, { recursive: true });

		const outputPath = path.join(outputDir, "classification-tag-definition-candidates.json");
		...
		console.log(`✅ saved ${result.length} classification tag candidates to ${outputPath}`);
...
main().catch((error) => {
  console.error("extract-classification-tag-definition-candidates failed");
  console.error(error);
  process.exit(1);
});
