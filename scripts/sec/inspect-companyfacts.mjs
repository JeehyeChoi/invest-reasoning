...
async function main() {
  ...
  const outputs = [
    ...
    {
      filename: "tag-catalog.json",
      data: tagCatalog,
    },
    {
      filename: "tag-distribution.json",
      data: tagDistribution,
    },
    {
      filename: "top-tags.json",
      data: topTags,
    },
  ];
  ...
  for (const output of outputs) {
    console.log(`- data/sec/inspection/companyfacts/${output.filename}`);
  }
}
...
function printHelpAndExit() {
  console.log(`
Usage:
  node scripts/sec/inspect-companyfacts.mjs [options]
