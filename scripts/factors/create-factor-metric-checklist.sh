...
FEATURE_DEFINITION_SQL_MARK=$(mark_required_grep \
  "$FEATURE_DEFINITION_FILE" \
  "'${FACTOR}', '${AXIS}', '${METRIC_KEY}'"
)
METRIC_DISPLAY_SQL_MARK=$(mark_required_grep \
  "$METRIC_DISPLAY_DEFINITION_FILE" \
  "'${FACTOR}', '${AXIS}', '${METRIC_KEY}'"
)
AXIS_DISPLAY_SQL_MARK=$(mark_required_grep \
  "$AXIS_DISPLAY_DEFINITION_FILE" \
  "'${FACTOR}', '${AXIS}'"
)

if [ "$AXIS" = "fundamentals_based" ]; then
  METRIC_REGISTRY_FILE="src/shared/sec/metrics.ts"
  METRIC_REGISTRY_LABEL="shared SEC metric registry"
...
elif [ "$AXIS" = "market_price" ]; then
  METRIC_REGISTRY_FILE="src/shared/factors/marketPriceMetrics.ts"
  METRIC_REGISTRY_LABEL="shared market price metric registry"
  METRIC_REGISTRY_MARK=$(mark_required_grep \
    "$METRIC_REGISTRY_FILE" \
    "\"${METRIC_KEY}\""
  )
...
elif [ "$AXIS" = "valuation" ]; then
  METRIC_REGISTRY_FILE="src/shared/factors/valuationMetrics.ts"
  METRIC_REGISTRY_LABEL="shared valuation metric registry"
  METRIC_REGISTRY_MARK=$(mark_required_grep \
    "$METRIC_REGISTRY_FILE" \
    "\"${METRIC_KEY}\""
  )
  ...
  API_MARK="[n/a]"
  METRIC_RELIABILITY_OUTPUTS_MARK="[n/a]"
elif [ "$AXIS" = "etf_exposure" ]; then
  METRIC_REGISTRY_FILE="src/shared/factors/etfExposureMetrics.ts"
  METRIC_REGISTRY_LABEL="shared ETF exposure metric registry"
  METRIC_REGISTRY_MARK=$(mark_required_grep \
    "$METRIC_REGISTRY_FILE" \
    "\"${METRIC_KEY}\""
  )
  ...
  API_MARK="[n/a]"
  METRIC_RELIABILITY_OUTPUTS_MARK="[n/a]"
elif [ "$AXIS" = "macro_linked" ]; then
  METRIC_REGISTRY_FILE="src/shared/factors/macroLinkedMetrics.ts"
  METRIC_REGISTRY_LABEL="shared macro linked metric registry"
  METRIC_REGISTRY_MARK=$(mark_required_grep \
    "$METRIC_REGISTRY_FILE" \
    "\"${METRIC_KEY}\""
  )
  ...
  METRIC_REGISTRY_FILE="[n/a]"
  METRIC_REGISTRY_LABEL="shared metric registry"
  METRIC_REGISTRY_MARK="[n/a]"
  BACKEND_METRIC_DEFINITION_MARK="[n/a]"
  API_MARK="[n/a]"
  METRIC_RELIABILITY_OUTPUTS_MARK="[n/a]"
fi
...
## Definition SQL
- ${FEATURE_DEFINITION_SQL_MARK} ${FEATURE_DEFINITION_FILE} includes ${FACTOR}/${AXIS}/${METRIC_KEY}
- ${METRIC_DISPLAY_SQL_MARK} ${METRIC_DISPLAY_DEFINITION_FILE} includes ${FACTOR}/${AXIS}/${METRIC_KEY}
- ${AXIS_DISPLAY_SQL_MARK} ${AXIS_DISPLAY_DEFINITION_FILE} includes ${FACTOR}/${AXIS}

## Shared/backend type checks
- ${METRIC_REGISTRY_MARK} ${METRIC_REGISTRY_LABEL} includes ${METRIC_KEY} (${METRIC_REGISTRY_FILE})
...
- ${FEATURE_DEFINITION_OWNERSHIP_MARK} DB feature definition rows are not reused by another factor
...
## AI review
- [review] db/ticker_factor_metric_display_definitions.sql is filled if empty, then labels, chart title, and description match the metric and factor context.
- [review] db/ticker_factor_feature_definitions.sql is filled if empty, then feature mapping from enriched source columns is coherent.
- [review] db/ticker_factor_signal_definitions.sql includes the ${FACTOR}/${AXIS} candidate signals, priority order, selection rules, evidence rule, and confidence rule needed by this metric.
- [review] db/ticker_factor_feature_definitions.sql follows the normalized SQL schema: model_key, model_version, factor, axis, metric_key, feature_key, method/source columns, payloads, and usage flags; each feature_key is the persisted feature key.
- [review] Treat ${METRIC_KEY} as a reusable canonical input series and each feature as ${FACTOR}/${AXIS}-owned numeric evidence; signal definitions are the factor-axis state/label layer, not the feature layer.
- [review] If ${METRIC_KEY} is a highly processed derived metric that maps nearly one-to-one to a displayed feature, keep it in the metric layer when it is reusable across factors, axes, signals, charts, or clustering.
- [review] For valuation derived metrics, db/ticker_factor_feature_definitions.sql should usually use source=metric_value, method=direct, series.table=ticker_derived_metric_series, series.version=derived_metric_series_v0, and an explicit series.processKey matching the derived metric producer.
- [review] For valuation derived metrics, confirm src/backend/services/valuation/factors/buildTickerDerivedMetricSeries.ts writes the same derived_metric_key, process_key, value_type, period_end, effective_date, and source_payload formula described by db/ticker_factor_feature_definitions.sql.
- [review] For macro_linked metrics, db/ticker_factor_feature_definitions.sql should explicitly name the FRED macro source in source_payload/definition_payload and the executor should align company quarterly series to macro observation dates before calculating sensitivity.
- [review] For macro_linked metrics, confirm the selected FRED series key exists in src/backend/services/macro/fred/macroFredSeriesRegistry.ts and stored observations exist before accepting feature output gaps as real absence.
- [review] Every feature in db/ticker_factor_feature_definitions.sql defines its own series metadata (table, version, metricKey, periodType); do not rely on a top-level source default.
- [review] db/ticker_factor_feature_definitions.sql lists only active feature definitions; remove inactive candidates instead of keeping boolean enablement flags.
- [review] Each db/ticker_factor_feature_definitions.sql feature explicitly defines method, valueType, comparison, macroContrast, and clustering; omit reference only when the method does not need a comparison source.
...
- [review] Every source/reference/sources/denominator.source/counterpart.source column used by db/ticker_factor_feature_definitions.sql exists in db/sec_companyfact_metric_series_enriched.sql and is produced by src/backend/services/sec/companyFacts/series/enriched/buildCompanyFactsMetricSeriesEnrichedForCik.ts; add enriched fields there first when a new feature method needs a new calculation basis.
- [review] Every source/reference/sources/denominator.source/counterpart.source column used by db/ticker_factor_feature_definitions.sql produces non-null values for the metric's actual factType and periodType; column existence alone is not enough.
...
- [review] If db/ticker_factor_feature_definitions.sql uses any duration_adjusted_* source/reference, confirm src/backend/config/sec/metrics.ts sets ${METRIC_KEY} durationPolicy=duration_adjust_growth; reported_only and not_applicable metrics should use reported/point fields that are actually produced.
- [review] db/ticker_factor_feature_definitions.sql may reuse the metric as an input, but each feature definition remains ${FACTOR}/${AXIS}-owned unless it is an explicit shared concept such as turnaroundMomentum; do not accidentally reuse the same source/method/reference/lookback/sources feature definition from another factor-axis.
...
- [review] If tagMeta.ts factType and db/ticker_factor_feature_definitions.sql source.periodType differ, stop and either change db/ticker_factor_feature_definitions.sql to the correct period type or implement the missing enriched/feature semantics first; do not rely on empty feature output as an acceptable fallback.
...
- [review] src/backend/services/sec/companyFacts/series/signal/buildTickerFactorSignals.ts covers the ${FACTOR}/${AXIS} signal selection path using ticker_factor_signal_definitions, including observed metric counts, confidence, and supporting evidence for the feature methods used by db/ticker_factor_feature_definitions.sql.
...
## Notes
- Definition SQL files are scaffold-checked by grep, but display content and feature mapping should be reviewed in the AI review step.
...
- active.ts resolves display config from DB; factor metric calculation uses blueprint scope and DB feature definitions.
...
- For fundamentals_based metrics, run metric_series, sec_metric_series_enriched, fundamentals_based_factor_features, and factor_signals. For valuation features, ensure both SEC metric series and price history are current, then run derived_metric_series, valuation_factor_features, and factor_signals. For market_price metrics, run ticker_daily_price_history_sync as needed, then market_price_factor_features and factor_signals.
