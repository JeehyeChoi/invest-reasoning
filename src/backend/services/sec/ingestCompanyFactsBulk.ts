...
import { downloadSecBulkArchiveToFile } from "@/backend/services/sec/downloadSecBulkArchiveToFile";
import { ensureFreshSecBulkArchive } from "@/backend/services/sec/ensureFreshSecBulkArchive";

import { loadCompanyFactsStateMap } from "@/backend/services/sec/loadCompanyFactsStateMap";
import { mapTickersToCiks } from "@/backend/services/sec/mapTickersToCiks";
import { scanCompanyFactsZipEntries } from "@/backend/services/sec/scanCompanyFactsZipEntries";
import { processCompanyFactsZipEntries } from "@/backend/services/sec/processCompanyFactsZipEntries";

import { flattenCompanyFacts } from "@/backend/services/sec/flattenCompanyFacts";
import { upsertSecCompanyFactRows } from "@/backend/services/sec/upsertSecCompanyFactRows";
import { upsertSecCompanyFactCompanyState } from "@/backend/services/sec/upsertSecCompanyFactCompanyState";

import {
  getSecBulkIngestState,
  upsertSecBulkIngestState,
} from "@/backend/services/sec/secBulkIngestStateRepository";
...
export async function ingestCompanyFactsBulk(): Promise<CompanyFactsBulkIngestResult> {
...
console.log("===== [SEC BULK DEBUG END] =====\n");
  ...
  try {
    ...
    const sp500Json = JSON.parse(
      fs.readFileSync(sp500Path, "utf-8")
    ) as TickerUniverseFile;

    const universeTickers = Array.isArray(sp500Json.tickers)
      ? sp500Json.tickers
      : [];

    const allowedCiks = await mapTickersToCiks(universeTickers);
    ...
    console.log(
      `[SEC BULK] Universe tickers: ${universeTickers.length}, allowed CIKs: ${allowedCiks.size}`
    );
    console.log(
