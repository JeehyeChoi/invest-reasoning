...
type FundamentalGrowthMetricStepDeps<TSeriesPoint, TMetrics, TConfig = unknown> = {
  factor: FactorKey;
...
export async function runFundamentalGrowthMetricStep<
  TSeriesPoint,
  TMetrics,
  TConfig = unknown,
>(
  state: TickerFactorSnapshotWorkflowState,
  deps: FundamentalGrowthMetricStepDeps<TSeriesPoint, TMetrics, TConfig>,
  ...
  for (const ticker of state.tickers) {
    ...
    try {
      ...
      deps.assignInput(state, ticker, deps.metricKey, metrics);
