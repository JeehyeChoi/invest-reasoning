  ...
  return (
    <Panel title="Metric Breakdown">
      {metric ? (
        ...
        <p className="font-mono text-sm">No headline metric available.</p>
      )}
    </Panel>
  );
}
...
function buildFormulaText(metric: TickerOverviewFactorMetric): string {
  if (!metric.metrics) {
    return "-";
  }

  const entries = Object.entries(metric.metrics).filter(([key]) => key !== "score");
  if (entries.length === 0) {
    return "-";
  }

  const knownRevenueWeights: Record<string, number> =
    metric.factor === "growth" &&
    metric.axis === "fundamentals_based" &&
  ...
  const weightedTerms = Object.entries(knownRevenueWeights).map(
    ([key, weight]) => `${weight}·${formatLabel(key)}`,
  );
  ...
  return entries.map(([key]) => formatLabel(key)).join(" + ");
}
